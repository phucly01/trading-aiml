"""
The code in this class was taken from https://github.com/x4nth055/pythoncode-tutorials/blob/master/machine-learning/stock-prediction
and was converted to class object.  It was used only for educational purposes and comparison to my other implementations.  
"""
...
class Rockikz_x4nth055:
    ...
    def load_data(self, ticker, n_steps=50, scale=True, shuffle=True, lookup_step=1, split_by_date=True,
                test_size=0.2, feature_columns=['close', 'volume', 'open', 'high', 'low']) -> dict:
        """
        ...
        # result["test_df"] = result["df"].loc[dates] #This isn't working, use the line below instead
    ...
    def train(self, dataframe):
        ...
        global model_name
        model_name = "Test"
        global ticker_data_filename
        ticker_data_filename = "test.csv"
        ...
        # load the data
        data = self.load_data(ticker, N_STEPS, scale=SCALE, split_by_date=SPLIT_BY_DATE, 
                        shuffle=SHUFFLE, lookup_step=LOOKUP_STEP, test_size=TEST_SIZE, 
                        feature_columns=FEATURE_COLUMNS)

        # save the dataframe
        print("Saving to {}".format(ticker_data_filename))
        data["df"].to_csv(ticker_data_filename)

        # construct the model
        model = self.create_model(N_STEPS, len(FEATURE_COLUMNS), loss=LOSS, units=UNITS, cell=CELL, n_layers=N_LAYERS,
    ...
    def plot_graph(self, test_df):
        ...
        plt.plot(test_df["date"], test_df[f'true_close_{LOOKUP_STEP}'], c='b')
        plt.plot(test_df["date"], test_df[f'close_{LOOKUP_STEP}'], c='r')
        plt.xlim(xmin=datetime.datetime(2023, 12, 1))
        plt.xlabel("Date")
        plt.ylabel("Price")
